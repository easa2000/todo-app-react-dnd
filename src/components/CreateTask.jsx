...
const CreateTask = ({tasks, setTasks}) => {
    ...
    const handleSubmit = (e) => {
        ...
        if(task.name.length > 100) return toast.error("Atask must not be more than 100 characters");
    ...
    return ( 
        <form onSubmit={handleSubmit}>
            <input type="text" className="border-2 border-slate-400 bg-slate-100 rounded-md mr-4 h-12 w-64 px-1 mt-3" 
            value={task.name} 
